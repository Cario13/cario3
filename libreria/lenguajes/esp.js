exports.panel = (prefix, time, fechamex, fechabol, fechachi, fechaarg, fechacol, fechaecu, fechacosr, fechacub, fechagua, fechahon, fechanic, fechapan, fechauru, fechaven, fechapar) => {
  return `*╠「 Zona-Horaria 🌎 」*
*║Peru            : ${time}*
*║Mexico        : ${fechamex}*
*║Bolivia         : ${fechabol}*
*║Chile            : ${fechachi}*
*║Argentina    : ${fechaarg}*
*║Colombia    : ${fechacol}*
*║Ecuador       : ${fechaecu}*
*║Costa_Rica : ${fechacosr}*
*║Cuba            : ${fechacub}*
*║Guatemala  : ${fechagua}*
*║Honduras    : ${fechahon}*
*║Nicaragua   : ${fechanic}*
*║Panamá      : ${fechapan}*
*║Uruguay      : ${fechauru}*
*║Venezuela   : ${fechaven}*
*║Paraguay    : ${fechapar}*
*╚═══════════*
*╭┈┈┈┈┈┈┈┈┈┈┈*
➱ *_COMANDOS_  ☷*
*╰┈┈┈┈┈┈┈┈┈┈┈*
*╔═══════════*
*║ _CONVERSORES :_*
*║╭—————————*
*║├${prefix}sticker*
*║├${prefix}aimg*
*║├${prefix}acelerarvid*
*║├${prefix}lentovid*
*║├${prefix}reversavid*
*║├${prefix}amp3*
*║├${prefix}inframundo*
*║├${prefix}happyhardcore*
*║├${prefix}ardilla*
*║├${prefix}superveloz*
*║├${prefix}bruhbajo*
*║├${prefix}nightcore*
*║├${prefix}lento*
*║├${prefix}demonio*
*║├${prefix}distorcionado*
*║├${prefix}aumentarbajo*
*║├${prefix}grabacion*
*║╰—————————*
*╚═══════════*
*╔═══════════*
*║ _CMDS ADMINS :_*
*║╭—————————*
*║├${prefix}deathnote*
*║├${prefix}cgpen*
*║├${prefix}agp*
*║├${prefix}editnamegp*
*║├${prefix}editdesgp*
*║├${prefix}fotogrupo*
*║├${prefix}invocar*
*║├${prefix}invocar2*
*║├${prefix}invocar3*
*║├${prefix}invocar4*
*║├${prefix}ascender*
*║├${prefix}degradar*
*║├${prefix}nivelear*
*║├${prefix}banchat*
*║├${prefix}antilink*
*║├${prefix}antilink2*
*║├${prefix}wlc*
*║├${prefix}modofull*
*║├${prefix}animeuwu*
*║├${prefix}hmodo*
*║├${prefix}antifakes*
*║├${prefix}antiestranjeros*
*║├${prefix}l*
*║├${prefix}.*
*║├${prefix}buscamsj*
*║├${prefix}delttc*
*║╰—————————*
*╚═══════════*
*╔═══════════*
*║ _CMDS CREADOR :_*
*║╭—————————*
*║├${prefix}reiniciarlimite*
*║├${prefix}prefijoedit*
*║├${prefix}suprimir*
*║├${prefix}desterrar*
*║├${prefix}ban*
*║├${prefix}bc*
*║├${prefix}modo1*
*║├${prefix}modo2*
*║├${prefix}antidel*
*║├${prefix}leerchat*
*║├${prefix}banear*
*║├${prefix}fotocloner*
*║├${prefix}perfilbot*
*║├${prefix}quitarbaneo*
*║├${prefix}mysession*
*║||| ~MODO KUAKER~ |||*
*║├${prefix}c1*
*║├${prefix}c2*
*║├${prefix}c3*
*║├${prefix}c4*
*║├${prefix}c5*
*║├${prefix}c6*
*║├${prefix}:)*
*║╰—————————*
*╚═══════════*
*╔═══════════*
*║ _PASATIEMPO :_*
*║╭—————————*
*║├${prefix}quien*
*║├${prefix}calumnia*
*║├${prefix}preguntame*
*║├${prefix}pregunta2*
*║├${prefix}ruleta*
*║├${prefix}testgay*
*║├${prefix}tragamoneda*
*║├${prefix}dado*
*║├${prefix}dados*
*║├${prefix}da2*
*║├${prefix}emparejar*
*║├${prefix}top5*
*║├${prefix}v_o_f*
*║├${prefix}ttc*
*║╰—————————*
*╚═══════════*
*╔═══════════*
*║ _OTROS CMDS :_*
*║╭—————————*
*║├${prefix}creador*
*║├${prefix}nuebot*
*║├${prefix}listnuebot*
*║├${prefix}gruposwa*
*║├${prefix}minivel*
*║├${prefix}wame*
*║├${prefix}infogrupo*
*║├${prefix}grupocreador*
*║├${prefix}tienda*
*║├${prefix}tiendaimg*
*║├${prefix}rescom*
*║├${prefix}milimite*
*║├${prefix}bloqueados*
*║├${prefix}admins*
*║├${prefix}grupolink*
*║├${prefix}ia_void*
*║├${prefix}estadobot*
*║├${prefix}frase*
*║├${prefix}miperfil*
*║╰—————————*
*╚═══════════*
*╔═══════════*
*║ _MAS COMANDOS :_*
*║╭—————————*
*║├${prefix}labiblia*
*║├${prefix}crealogos*
*║├${prefix}animages*
*║├${prefix}randmenu*
*║╰—————————*
*╚═══════════*
`
}
exports.panel2 = (prefix, time, fechamex, fechabol, fechachi, fechaarg, fechacol, fechaecu, fechacosr, fechacub, fechagua, fechahon, fechanic, fechapan, fechauru, fechaven, fechapar) => {
  return `*╔═══════════════*
*╠「 Zona-Horaria 🌎 」*
*║Peru            : ${time}*
*║Mexico        : ${fechamex}*
*║Bolivia         : ${fechabol}*
*║Chile            : ${fechachi}*
*║Argentina    : ${fechaarg}*
*║Colombia    : ${fechacol}*
*║Ecuador       : ${fechaecu}*
*║Costa_Rica : ${fechacosr}*
*║Cuba            : ${fechacub}*
*║Guatemala  : ${fechagua}*
*║Honduras    : ${fechahon}*
*║Nicaragua   : ${fechanic}*
*║Panamá      : ${fechapan}*
*║Uruguay      : ${fechauru}*
*║Venezuela   : ${fechaven}*
*║Paraguay    : ${fechapar}*
*╚═══════════════*
*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈*
➱ *_COMANDOS_  ☷*
*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈*
*╔═══════════════*
*║ _CONVERSORES :_*
*║╭————————————*
*║├${prefix}sticker* _[convierte una imagen a sticker]_
*║├${prefix}aimg* _[convierte un sticker a imagen]_
*║├${prefix}acelerarvid* _[aumentar velocidad de un vídeo]_
*║├${prefix}lentovid* _[reducir velocidad de video]_
*║├${prefix}reversavid* _[invierte un vídeo]_
*║├${prefix}amp3* _[convierte video a audio]_
*║├${prefix}inframundo* _[editor de audio]_
*║├${prefix}happyhardcore* _[editor de audio]_
*║├${prefix}ardilla* _[editor de audio]_
*║├${prefix}superveloz* _[editor de audio]_
*║├${prefix}bruhbajo* _[editor de audio]_
*║├${prefix}nightcore* _[editor de audio]_
*║├${prefix}lento* _[editor de audio]_
*║├${prefix}demonio* _[editor de audio]_
*║├${prefix}distorcionado* _[editor de audio]_
*║├${prefix}aumentarbajo* _[editor de audio]_
*║├${prefix}grabacion* _[editor de audio]_
*║╰————————————*
*╚═══════════════*
*╔═══════════════*
*║ _CMDS ADMINS :_*
*║╭————————————*
*║├${prefix}deathnote* _[eliminacion random]_
*║├${prefix}cgpen* _[cerrar grupo temporizado]_
*║├${prefix}agp* _[abrir grupo]_
*║├${prefix}editnamegp* _[cambiar nombre al grupo]_
*║├${prefix}editdesgp* _[cambiar descripción]_
*║├${prefix}fotogrupo* _[cambiar foto del grupo]_
*║├${prefix}invocar* _[etiqueta a todos]_
*║├${prefix}invocar2* _[etiqueta a todos]_
*║├${prefix}invocar3* _[etiqueta a todos]_
*║├${prefix}invocar4* _[etiqueta a todos]_
*║├${prefix}ascender* _[convertir a administrador]_
*║├${prefix}degradar* _[quitar administracion]_
*║├${prefix}nivelear* _[activar niveles]_
*║├${prefix}banchat* _[banear un chat]_
*║├${prefix}antilink* _[anti-link normal]_
*║├${prefix}antilink2* _[anti-link multiple]_
*║├${prefix}wlc* _[bienvenida automática]_
*║├${prefix}modofull* _[activador 1]_
*║├${prefix}animeuwu* _[activador 2]_
*║├${prefix}hmodo* _[activador 3]_
*║├${prefix}antifakes* _[elimina números fakes]_
*║├${prefix}antifakes2* _[eliminar números +1]_
*║├${prefix}antiestranjeros* _[elimina números estranjeros]_
*║├${prefix}l* _[etiqueta multimedia]_
*║├${prefix}.* _[etiqueta invisible]_
*║├${prefix}buscamsj* _[busca un mensaje en el chat]_
*║├${prefix}delttc* _[eliminar un desafio tictactoe]_
*║╰————————————*
*╚═══════════════*
*╔═══════════════*
*║ _CMDS CREADOR :_*
*║╭————————————*
*║├${prefix}reiniciarlimite* _[reinicia el limite]_
*║├${prefix}prefijoedit* _[editar prefijo nuevo]_
*║├${prefix}suprimir* _[eliminar un mensaje del bot]_
*║├${prefix}desterrar* _~[eliminar con etiqueta]~_
*║├${prefix}ban* _~[eliminar con mención]~_
*║├${prefix}bc* _[transmitir un mensaje]_
*║├${prefix}modo1* _[modo privado/publico]_
*║├${prefix}modo2* _[modo enlinea/sinlinea]_
*║├${prefix}leerchat* _[leer chats automáticamente]_
*║├${prefix}antidel* _[anti eliminar]_
*║├${prefix}banear* _[banear a un cliente]_
*║├${prefix}fotocloner* _[foto de usuario al bot]_
*║├${prefix}perfilbot* _[cambiar foto del bot]_
*║├${prefix}quitarbaneo* _[quitar baneo]_
*║├${prefix}mysession* _[código WaSession]_
*║||| ~MODO KUAKER~ |||*
*║[ ! ] Simples traba grupo nwn [ ! ]*
*║ ~PSDT- Estos comandos son de virustraba~*
*║ ~ten cuidado al usarlos~*
*║├${prefix}c1* _[grupo crash 1]_
*║├${prefix}c2* _[grupo crash 2]_
*║├${prefix}c3* _[grupo crash 3]_
*║├${prefix}c4* _[grupo crash 4]_
*║├${prefix}c5* _[grupo crash 5]_
*║├${prefix}c6* _[grupo crash 6]_
*║├${prefix}:)* _[multi crash]_
*║╰————————————*
*╚═══════════════*
*╔═══════════════*
*║ _PASATIEMPO :_*
*║╭————————————*
*║├${prefix}quien* _[seleccion aleatorio]_
*║├${prefix}calumnia* _[crea un mensaje falso]_
*║├${prefix}preguntame* _[preguntas random]_
*║├${prefix}pregunta2* _[preguntas otaku]_
*║├${prefix}ruleta* _[juego desesperante]_
*║├${prefix}testgay* _[porcentaje gey]_
*║├${prefix}tragamoneda* _[tragamoneda :v]_
*║├${prefix}dado* _[dados 1]_
*║├${prefix}dados* _[dados 2]_
*║├${prefix}da2* _[dados 3]_
*║├${prefix}emparejar* _[crear una pareja]_
*║├${prefix}top5* _[crear tops]_
*║├${prefix}v_o_f* _[verdad o falso]_
*║├${prefix}ttc* _[tres en raya]_
*║╰————————————*
*╚═══════════════*
*╔═══════════════*
*║ _OTROS CMDS :_*
*║╭————————————*
*║├${prefix}nuebot* _[para convertirse en bot]_
*║├${prefix}listnuebot* _[lista de bots]_
*║├${prefix}gruposwa* _[grupos de WhatsApp]_
*║├${prefix}minivel* _[ver tu nivel actual]_
*║├${prefix}wame* _[link directo de tu número]_
*║├${prefix}infogrupo* _[información del grupo]_
*║├${prefix}grupocreador* _[Creador del grupo]_
*║├${prefix}tienda* _[genera una tienda falsa]_
*║├${prefix}tiendaimg* _[genera una tienda falsa]_
*║├${prefix}rescom* _[copia comentario de una imagen]_
*║├${prefix}milimite* _[ver tu límite actual]_
*║├${prefix}bloqueados* _[numeros bloqueados]_
*║├${prefix}admins* _[administradores del grupo]_
*║├${prefix}grupolink* _[link del grupo]_
*║├${prefix}ia_void* _[textvoid mucho texto]_
*║├${prefix}estadobot* _[ver el estado del bot]_
*║├${prefix}frase* _[frases random]_
*║├${prefix}miperfil* _[info de tu perfil]_
*║╰————————————*
*╚═══════════════*
*╔═══════════════*
*║ _MAS COMANDOS EXTRA :_*
*║╭————————————*
*║├${prefix}randmenu* _[mas comandos]_
*║├${prefix}labiblia* _[comandos H]_
*║├${prefix}crealogos* _[comandos de logos]_
*║├${prefix}animages* _[comandos anime]_
*║╰————————————*
*╚═══════════════*
`
}
exports.menunsfw = (prefix) => {
  return `😳 *MENU NSFW* 👌
 
 _*FULL H*_ 7v7r
╔══════════════╗
║➩ ❍ ${prefix}bj
║➩ ❍ ${prefix}ero
║➩ ❍ ${prefix}cum
║➩ ❍ ${prefix}feet
║➩ ❍ ${prefix}yuri
║➩ ❍ ${prefix}trap
║➩ ❍ ${prefix}lewd
║➩ ❍ ${prefix}feed
║➩ ❍ ${prefix}eron
║➩ ❍ ${prefix}solo
║➩ ❍ ${prefix}gasm
║➩ ❍ ${prefix}poke
║➩ ❍ ${prefix}anal
║➩ ❍ ${prefix}holo
║➩ ❍ ${prefix}tits
║➩ ❍ ${prefix}kuni
║➩ ❍ ${prefix}kiss
║➩ ❍ ${prefix}erog
║➩ ❍ ${prefix}smug
║➩ ❍ ${prefix}baka
║➩ ❍ ${prefix}solog
║➩ ❍ ${prefix}feetg
║➩ ❍ ${prefix}lewdk
║➩ ❍ ${prefix}pussy_jpg
║➩ ❍ ${prefix}femdom
║➩ ❍ ${prefix}cuddle
║➩ ❍ ${prefix}hwaifu
║➩ ❍ ${prefix}hneko
║➩ ❍ ${prefix}yaoi
║➩ ❍ ${prefix}ecchi
║➩ ❍ ${prefix}hentai
║➩ ❍ ${prefix}hentai2
║➩ ❍ ${prefix}ahegao
║➩ ❍ ${prefix}nekopoi
║➩ ❍ ${prefix}eroyuri
║➩ ❍ ${prefix}cum_jpg
║➩ ❍ ${prefix}erofeet
║➩ ❍ ${prefix}holoero
║➩ ❍ ${prefix}classic
║➩ ❍ ${prefix}fox_girl
║➩ ❍ ${prefix}erokemo
║➩ ❍ ${prefix}blowjob
║➩ ❍ ${prefix}hololewd
║➩ ❍ ${prefix}sideoppai
║➩ ❍ ${prefix}futanari
║➩ ❍ ${prefix}lewdkemo
║➩ ❍ ${prefix}wallpaper
║➩ ❍ ${prefix}animefeets
║➩ ❍ ${prefix}animebooty
║➩ ❍ ${prefix}pussy_jpg
║➩ ❍ ${prefix}chiisaihentai
║➩ ❍ ${prefix}animethighss
║➩ ❍ ${prefix}kemonomimi
║➩ ❍ ${prefix}nsfw_avatar
║➩ ❍ ${prefix}hentaiparadise
║➩ ❍ ${prefix}traph (trapito)
║➩ ❍ ${prefix}animearmpits
║➩ ❍ ${prefix}hentaifemdom
║➩ ❍ ${prefix}lewdanimegirls
║➩ ❍ ${prefix}biganimetiddies
║➩ ❍ ${prefix}animebellybutton
║➩ ❍ ${prefix}hentai4everyone
  _*STICKERS H*_
║➩ ❍ ${prefix}nsfw_neko_gif (sticker)
║➩ ❍ ${prefix}random_hentai_gif (sticker)
  _*USO DE CODIGOS NUCLEARES*_
║➩ ❍ ${prefix}nhentaibuscar
║➩ ❍ ${prefix}nhentaipdf
╚══════════════╝
  
  *iiPor favor no saturar los comandos!!*
  
  _Uso de comandos minimo 12 segundos en promedio_`
}
exports.menuarteuwu = (prefix) => {
  return `*[ Creacion de logos ]*
╔══════════════╗
║➩ ❍ ${prefix}gneon
║➩ ❍ ${prefix}logocosmic
║➩ ❍ ${prefix}wolflogo
║➩ ❍ ${prefix}text3d
║➩ ❍ ${prefix}logocosmic
║➩ ❍ ${prefix}fraseedit
║➩ ❍ ${prefix}shadow
║➩ ❍ ${prefix}cup
║➩ ❍ ${prefix}cup1
║➩ ❍ ${prefix}romance
║➩ ❍ ${prefix}smoke
║➩ ❍ ${prefix}burnpaper
║➩ ❍ ${prefix}lovemessage
║➩ ❍ ${prefix}undergrass
║➩ ❍ ${prefix}love
║➩ ❍ ${prefix}coffe
║➩ ❍ ${prefix}woodheart
║➩ ❍ ${prefix}woodenboard
║➩ ❍ ${prefix}summer3d
║➩ ❍ ${prefix}wolfmetal
║➩ ❍ ${prefix}nature3d
║➩ ❍ ${prefix}underwater
║➩ ❍ ${prefix}golderrose
║➩ ❍ ${prefix}summernature
║➩ ❍ ${prefix}letterleaves
║➩ ❍ ${prefix}glowingneon
║➩ ❍ ${prefix}fallleaves
║➩ ❍ ${prefix}flamming
║➩ ❍ ${prefix}harrypotter
║➩ ❍ ${prefix}carvedwood
║➩ ❍ ${prefix}blackpink
║➩ ❍ ${prefix}neon
║➩ ❍ ${prefix}greenneon
║➩ ❍ ${prefix}advanceglow
║➩ ❍ ${prefix}futureneon
║➩ ❍ ${prefix}sandwriting
║➩ ❍ ${prefix}sandsummer
║➩ ❍ ${prefix}sandengraved
║➩ ❍ ${prefix}metaldark
║➩ ❍ ${prefix}neonlight
║➩ ❍ ${prefix}holographic
║➩ ❍ ${prefix}text1917
║➩ ❍ ${prefix}minion
║➩ ❍ ${prefix}deluxesilver
║➩ ❍ ${prefix}newyearcard
║➩ ❍ ${prefix}bloodfrosted
║➩ ❍ ${prefix}halloween
║➩ ❍ ${prefix}jokerlogo
║➩ ❍ ${prefix}fireworksparkle
║➩ ❍ ${prefix}natureleaves
║➩ ❍ ${prefix}bokeh
║➩ ❍ ${prefix}toxic
║➩ ❍ ${prefix}strawberry
║➩ ❍ ${prefix}box3d
║➩ ❍ ${prefix}roadwarning
║➩ ❍ ${prefix}breakwall
║➩ ❍ ${prefix}icecold
║➩ ❍ ${prefix}luxury
║➩ ❍ ${prefix}cloud
║➩ ❍ ${prefix}summersand
║➩ ❍ ${prefix}horrorblood
║➩ ❍ ${prefix}thunder
║➩ ❍ ${prefix}bannerlol
║➩ ❍ ${prefix}arcade8bit
║➩ ❍ ${prefix}writegalacy
║➩ ❍ ${prefix}textbyname
║➩ ❍ ${prefix}galaxybat
║➩ ❍ ${prefix}snow3d
║➩ ❍ ${prefix}birthdayday
║➩ ❍ ${prefix}goldplaybutton
║➩ ❍ ${prefix}silverplaybutton
║➩ ❍ ${prefix}wallgravity
║➩ ❍ ${prefix}steel3d
║➩ ❍ ${prefix}lionlogo
║➩ ❍ ${prefix}ninjalogo
║➩ ❍ ${prefix}marvelstudio
║➩ ❍ ${prefix}space
║➩ ❍ ${prefix}avenger
║➩ ❍ ${prefix}wooden3d
║➩ ❍ ${prefix}starsnight
║➩ ❍ ${prefix}textcake
║➩ ❍ ${prefix}glittergold
║➩ ❍ ${prefix}metallogo
║➩ ❍ ${prefix}noeltext
║➩ ❍ ${prefix}greenbush
║➩ ❍ ${prefix}glossychrome
║➩ ❍ ${prefix}greenneon
║➩ ❍ ${prefix}hologram3d
║➩ ❍ ${prefix}galaxystyle
║➩ ❍ ${prefix}birthdaycake
║➩ ❍ ${prefix}heartshaped
║➩ ❍ ${prefix}royaltext
║➩ ❍ ${prefix}puppycute
║➩ ❍ ${prefix}beautifulflower
║➩ ❍ ${prefix}lighttext
║➩ ❍ ${prefix}galaxywallpaper
║➩ ❍ ${prefix}luxurygold
║➩ ❍ ${prefix}watercolor
║➩ ❍ ${prefix}multicolor3d
║➩ ❍ ${prefix}wetglass
║➩ ❍ ${prefix}pubg
║➩ ❍ ${prefix}battlefield4
║➩ ❍ ${prefix}pornhub
║➩ ❍ ${prefix}glitch
╚══════════════╝
𝙿𝚘𝚛 𝚎𝚕 𝚕𝚒𝚖𝚒𝚝𝚊𝚍𝚘 𝚙𝚛𝚘𝚌𝚎𝚜𝚊𝚖𝚒𝚎𝚗𝚝𝚘 𝚍𝚎 𝚕𝚘𝚜 𝚖𝚘𝚟𝚒𝚕𝚎𝚜, 𝚙𝚞𝚎𝚍𝚎 𝚚𝚞𝚎 𝚊𝚕𝚐𝚞𝚗𝚊𝚜 𝚒𝚖𝚊𝚐𝚎𝚗𝚎𝚜 𝚎𝚍𝚒𝚝𝚊𝚍𝚊𝚜 𝚘 𝚟𝚒𝚍𝚎𝚘𝚜 𝚗𝚘 𝚜𝚎𝚊𝚗 𝚙𝚛𝚘𝚌𝚎𝚜𝚊𝚍𝚊𝚜 𝚍𝚎 𝚖𝚊𝚗𝚎𝚛𝚊 𝚌𝚘𝚛𝚛𝚎𝚌𝚝𝚊 𝚢 𝚗𝚘 𝚛𝚎𝚌𝚒𝚋𝚊𝚗 𝚗𝚊𝚍𝚊 𝚍𝚎 𝚋𝚘𝚝
  
  _*uso de comandos en un lapso de 10s minimo*_`
}
exports.menuimagenes = (prefix) => {
  return `*Full imagenes animes*

╔══════════════╗
║➩ ❍ ${prefix}ultimoanimes
║➩ ❍ ${prefix}kawaii1
║➩ ❍ ${prefix}kawaii2
║➩ ❍ ${prefix}kawaii3
║➩ ❍ ${prefix}stickbaka
║➩ ❍ ${prefix}loli
║➩ ❍ ${prefix}nekos
║➩ ❍ ${prefix}waifus
║➩ ❍ ${prefix}animeme
║➩ ❍ ${prefix}art
║➩ ❍ ${prefix}elf
║➩ ❍ ${prefix}exo
║➩ ❍ ${prefix}loli2
║➩ ❍ ${prefix}neko
║➩ ❍ ${prefix}waifu
║➩ ❍ ${prefix}shota
║➩ ❍ ${prefix}husbu
║➩ ❍ ${prefix}sagiri
║➩ ❍ ${prefix}shinobu
║➩ ❍ ${prefix}megumin
║➩ ❍ ${prefix}wallnime
║➩ ❍ ${prefix}heroeml
╚══════════════╝
𝙿𝚘𝚛 𝚎𝚕 𝚕𝚒𝚖𝚒𝚝𝚊𝚍𝚘 𝚙𝚛𝚘𝚌𝚎𝚜𝚊𝚖𝚒𝚎𝚗𝚝𝚘 𝚍𝚎 𝚕𝚘𝚜 𝚖𝚘𝚟𝚒𝚕𝚎𝚜, 𝚙𝚞𝚎𝚍𝚎 𝚚𝚞𝚎 𝚊𝚕𝚐𝚞𝚗𝚊𝚜 𝚒𝚖𝚊𝚐𝚎𝚗𝚎𝚜 𝚗𝚘 𝚜𝚎𝚊𝚗 𝚙𝚛𝚘𝚌𝚎𝚜𝚊𝚍𝚊𝚜 𝚍𝚎 𝚖𝚊𝚗𝚎𝚛𝚊 𝚌𝚘𝚛𝚛𝚎𝚌𝚝𝚊 𝚢 𝚗𝚘 𝚛𝚎𝚌𝚒𝚋𝚊𝚗 𝚗𝚊𝚍𝚊 𝚍𝚎 𝚋𝚘𝚝
  
  _*uso de comandos en un lapso de 10s minimo*_`
}
exports.mascomandos = (prefix) => {
  return `*RANDOMENU*

╔══════════════╗
║➩ ❍ ${prefix}top10
║➩ ❍ ${prefix}bandera?
║➩ ❍ ${prefix}amongus
║➩ ❍ ${prefix}simi
║➩ ❍ ${prefix}voz
║➩ ❍ ${prefix}audio
║➩ ❍ ${prefix}play
║➩ ❍ ${prefix}play2
║➩ ❍ ${prefix}youtube
║➩ ❍ ${prefix}playvid
║➩ ❍ ${prefix}letra
║➩ ❍ ${prefix}randimg
║➩ ❍ ${prefix}reconime
║➩ ❍ ${prefix}djbot
║➩ ❍ ${prefix}playstore
║➩ ❍ ${prefix}google
║➩ ❍ ${prefix}imagen
║➩ ❍ ${prefix}pinterest
║➩ ❍ ${prefix}ledstick
║➩ ❍ ${prefix}memerandom
║➩ ❍ ${prefix}ttp /2/3/4
║➩ ❍ ${prefix}xdnot
║➩ ❍ ${prefix}xd
║➩ ❍ ${prefix}semoji
║➩ ❍ ${prefix}clima
║➩ ❍ ${prefix}wattpadbuscar
║➩ ❍ ${prefix}qrcode
║➩ ❍ ${prefix}nombrerandom
║➩ ❍ ${prefix}infotik
║➩ ❍ ${prefix}stickerwa
║➩ ❍ ${prefix}telesticker
║➩ ❍ ${prefix}escritoriowpp
║➩ ❍ ${prefix}escritoriowpp2
║➩ ❍ ${prefix}tesla
║➩ ❍ ${prefix}nekosmic
║➩ ❍ ${prefix}videtexto
║➩ ❍ ${prefix}creavideoxd
║➩ ❍ ${prefix}fakedonald
║➩ ❍ ${prefix}minidatos
║➩ ❍ ${prefix}frasesamor
║➩ ❍ ${prefix}miname
╚══════════════╝
𝙿𝚘𝚛 𝚎𝚕 𝚕𝚒𝚖𝚒𝚝𝚊𝚍𝚘 𝚙𝚛𝚘𝚌𝚎𝚜𝚊𝚖𝚒𝚎𝚗𝚝𝚘 𝚍𝚎 𝚕𝚘𝚜 𝚖𝚘𝚟𝚒𝚕𝚎𝚜, 𝚙𝚞𝚎𝚍𝚎 𝚚𝚞𝚎 𝚊𝚕𝚐𝚞𝚗𝚊𝚜 𝚒𝚖𝚊𝚐𝚎𝚗𝚎𝚜 𝚗𝚘 𝚜𝚎𝚊𝚗 𝚙𝚛𝚘𝚌𝚎𝚜𝚊𝚍𝚊𝚜 𝚍𝚎 𝚖𝚊𝚗𝚎𝚛𝚊 𝚌𝚘𝚛𝚛𝚎𝚌𝚝𝚊 𝚢 𝚗𝚘 𝚛𝚎𝚌𝚒𝚋𝚊𝚗 𝚗𝚊𝚍𝚊 𝚍𝚎 𝚋𝚘𝚝
  
  _*uso de comandos en un lapso de 10s minimo*_`
}
exports.hora = () => {
  return `Horas`
}
exports.minuto = () => {
  return `Minutos`
}
exports.segundo = () => {
  return `Segundos`
}
exports.nospam = () => {
  return ` Por favor no sature al bot ;-;`
}
exports.nospam2 = (pushname) => {
  return ` *[ ! ] Mi king ${pushname}*\n\_Por favor espere unos segundos antes de usar otro comando_ ✓`
}
exports.opengp = () => {
  return `🔓 *[ GRUPO ABIERTO ]* 🔓`
}
exports.closegp = () => {
  return `🔒 *[ GRUPO CERRADO ]* 🔒`
}
exports.actualizacion1 = () => {
  return `*[ SE ACTUALIZO LA DESCRIPCIÓN DEL GRUPO ]*`
}
exports.actualizacion2 = () => {
  return `*[ AJUSTES REALIZADOS EL EN GRUPO ]*`
}
exports.actualizacion3 = () => {
  return `*[ AJUSTES REALIZADOS EL EN GRUPO ]*`
}
exports.Bienvenida = () => {
  return `Bienvenid@`
}
exports.Bienvenida2 = () => {
  return `a este grandioso grupo :`
}
exports.Bienvenida3 = () => {
  return `Por favor respete las normas del grupo para evitar inconvenientes :`
}
exports.admintrue = () => {
  return `Ahora sos un admin felicidades mi rey 😎🤝`
}
exports.adminfalse = () => {
  return `Ya no eres admin 😞🤙`
}
exports.despedida = () => {
  return `Adiuuu`
}
exports.nolink = () => {
  return `*[ ! ]* Se detecto un enlace\nVos eri un administrador, así que no te prohibiré el uso de enlaces ;)`
}
exports.nolink2 = () => {
  return `*Desterrado del grupo, motivo = Incumplir una regla*\n*[ ! ]* _enviar un link_ *[ ! ]*`
}
exports.wait = () => {
  return `_*Procesando, por favor espere...*_ ⌛`
}
exports.waitt = () => {
  return `_*Buscando resultado, espere...*_ 🔍`
}
exports.waittt = () => {
  return `*_Aguarde toy procesando su pedido..._*`
}
exports.success = () => {
  return `*[✔]Completado*`
}
exports.stick = () => {
  return `*[ ! ] _Lo siento, no se pudo convertir a sticker_*\n*[✓] _Intente nuevamente_*`
}
exports.Iv = () => {
  return `*[ X ] Link invalido [ X ]*`
}
exports.group = () => {
  return `*[ ! ] _Este comando solo se puede usar en grupos!_*`
}
exports.benned = () => {
  return `*[ ! ] _Estas baneado JIJIJA [ ! ]*`
}
exports.ownerG = () => {
  return `*[ ! ] _Este comando solo lo puede usar el creador del grupo!_*`
}
exports.ownerB = () => {
  return `*[ ! ] _Este comando solo lo puede usar el creador del bot!_*`
}
exports.admin = () => {
  return `*[ ! ] _Este comando solo puede ser usado por los administradores del grupo!_*`
}
exports.Badmin = () => {
  return `*[ ! ] _El bot debe ser ascendido a admin!_*`
}
exports.Bloqueads = () => {
  return `*_Lista de contactos bloqueados por el bot [✓]_* `
}
exports.ocrc = () => {
  return `*[ ! ] _Por favor use una imagen que contenga texto para poder escanearlo :)_* `
}
exports.converror = () => {
  return `*[ ! ] _Falló, en el momento de la conversión_* `
}
exports.converror2 = () => {
  return `*[ ! ] _Falló, se produjo un error, inténtelo de nuevo dentro de unos minutos_*`
}
exports.stickerror = (prefix) => {
  return `*[ ! ] _Envie/Responda una imagen usando el texto ${prefix}sticker_*`
}
exports.voztext = (prefix) => {
  return `*[ ! ] _Prefijo del idioma en el que desea escuchar el audio?_*\nEjemplo de uso: ${prefix}voz es hola\n\n_[✓] Use comando ${prefix}idiomas para ver los idiomas disponibles_`
}
exports.voztext2 = () => {
  return `*[ ! ] _Donde esta el texto?_*`
}
exports.voztext3 = () => {
  return `*[✓] _Limite de caracteres aceptados : 600_*`
}
exports.voztext4 = () => {
  return `*[ ! ] _No se pudo codificar el texto_*`
}
exports.voztext5 = () => {
  return `*Despues de usar el comando de vos, agregar un tipo de idioma, aqui los idiomas disponibles ✓* `
}
exports.predit = () => {
  return `*[ ! ] _Por favor use un solo simbolo o letra_*`
}
exports.predit2 = (prefix) => {
  return `*[✓] _El prefijo fue cambiado exitosamente_*\n_Prefijo nuevo :_ *${prefix}*`
}
exports.fakemsj = (prefix) => {
  return `Uso :\n${prefix}calumnia [@tag|mensaje|respuesta]]\n\nEjemplo : \n${prefix}calumnia @usuarioetiquetado|bendiceme|bendecido`
}
exports.alltag = () => {
  return `*Invocador* `
}
exports.alltag2 = () => {
  return ` *Invocando a los integrantes del grupo* `
}
exports.alltag3 = (prefix) => {
  return `Ejemplo de uso : ${prefix}. sexy quien lo lea 7w7`
}
exports.alltag4 = (prefix) => {
  return `Como usar:\n${prefix}l unvideo/unaimagen/unaudio...`
}
exports.Bcast = () => {
  return `*[ ! ] _Cual es el mensaje que desea transmitir?_*`
}
exports.Bcast2 = () => {
  return `⚡ *Mensaje simultaneo compartido con exito :D*⚡`
}
exports.admin1 = () => {
  return `*Un participante fue designado como administrador \n`
}
exports.admin2 = () => {
  return `*Se le ascendio a admin a* `
}
exports.admin3 = () => {
  return ` *ahora tiene el poder en el grupo! [✓]*`
}
exports.admin4 = () => {
  return `*[ ! ] _Etiquete a un participante que desee quitar su administración_* `
}
exports.admin5 = () => {
  return `Se descartó a un administrador \n*F*\n`
}
exports.admin6 = () => {
  return `*Degradacion de poder fue ejecutada por el administrador  :* `
}
exports.admin7 = () => {
  return `*Se le quito el poder a* `
}
exports.elimin = () => {
  return `*Etiqueta a un participante que desee eliminar* 🗡`
}
exports.admingod = () => {
  return `*Aqui la lista de los grandiosos admins del grupo* `
}
exports.gplinkk = () => {
  return `*[AQUI EL LINK DE ESTE GRANDIOSO GRUPO]* `
}
exports.saimg = () => {
  return `*[ ! ] _Responda un sticker!_*`
}
exports.saimg2 = () => {
  return `*[ ! ] _Me bugie, por favor intentalo nuevamente_* ^_^`
}
exports.notext = () => {
  return `Onta el texto?`
}
exports.wlcon = () => {
  return `*Bienvenida activado en este grupo [✓]* `
}
exports.wlcoff = () => {
  return `*Se desactivo la Bienvenida [ ! ]* `
}
exports.wlcon2 = () => {
  return `_*[Bienvenida AUTOMÁTICA]*_ `
}
exports.dlinkon = () => {
  return `Se activo el modo Anti-Link en el grupo `
}
exports.dlinkoff = () => {
  return `Se desactivo el modo Anti-Link en el grupo `
}
exports.dlink = () => {
  return `_*[MODO ANTILINK]*_ `
}
exports.activar = () => {
  return `< [ Activar `
}
exports.desactivar = () => {
  return `< [ Desactivar `
}
exports.mode = () => {
  return `*[ MODOS ] ✓*\n_PRIVADO / PUBLICO_ `
}
exports.mode2 = () => {
  return `PRIVADO 🔒`
}
exports.mode3 = () => {
  return `PUBLICO 🔓`
}
exports.mode4 = () => {
  return `*[ MODO PUBLICO ACTIVADO]* 🔓\n_Ahora todos los usuarios pueden usar al bot ✓_ `
}
exports.mode5 = () => {
  return `*[ MODO PRIVADO ACTIVADO]* 🔐\n_Ahora solo el dueño del bot puede usarlo ✓_`
}
exports.mode6 = () => {
  return `*||| ESTADO DEL BOT |||* `
}
exports.banear = () => {
  return `Baneo exitoso al numero : `
}
exports.banear2 = () => {
  return ` Ahora ya no podra usar ningun comando`
}
exports.banear3 = () => {
  return `》Lista de usuarios baneados《 : `
}
exports.banear4 = () => {
  return `Se le quito el baneo al numero `
}
exports.reg = (prefix) => {
  return `*[ ! ] ERROR!!! [ ! ]*\n\n_Por favor para completar el registro use el comando :_ ${prefix}rg tunombre|tuedad\n\nEjemplo: ${prefix}rg Matt|20`
}
exports.reg1 = (prefix) => {
  return `*[ ! ] No estas registrado en mi base de datos [ ! ]*\n\n_Por favor para registrarse use comando ${prefix}rg nombre|edad_\n\n*Ejm: ${prefix}rg Matt|19*`
}
exports.reg2 = () => {
  return `*Ya estuviste registrado en mi base de datos! ✓*`
}
exports.reg3 = (prefix) => {
  return `*_Mi king escriba bien su registro_*\n_Ejemplo:_\n*${prefix}rg Matt|20*`
}
exports.reg4 = () => {
  return `*Solo c acepta numeros para edad -.-!!!*`
}
exports.reg5 = () => {
  return `*Wtf muy largo tu nombre sos un alien ._.*\n_Nombre aceptable debe contener 10 digitos maximo_`
}
exports.reg6 = () => {
  return `*Tas muy viejo para usar este bot *`
}
exports.reg7 = () => {
  return `*Eres menor de edad, tienes permiso de tus padres para usar whatsapp?*`
}
exports.reg8 = (sender, pushname, rgNombre, rgEdad, time, serialUser) => {
  return `〘  *REGISTRO* 〙   ‏  ‌   ‏  ‌\n❥Fecha y hora de Registro \n❥${time}\n┏─━─━━─━─━━─━─\n╠≽️ *Nombre Registrado* : *${rgNombre}*\n╠≽️ *Nombre usado en whatsapp* : *${pushname}*\n╠≽️ *Edad* : *${rgEdad}*\n╠≽️ *Nº* : *wa.me/${sender.split("@")[0]}*\n┗─━─━━─━─━━─━─\n_Codigo de registro_ : *${serialUser}* `
}
exports.reg9 = (prefix) => {
  return `*_Tu Registro se completo con etsito, escriba ${prefix}menu para mostrar el panel de comandos_*`
}
exports.pint = () => {
  return `*[ ! ] _Que desea buscar en pinterest?_*`
}
exports.gugul = () => {
  return `*[ ! ] _Que imagen desea buscar!?_*`
}
exports.storeapp = () => {
  return `Que aplicación desea buscar en la PlayStore?`
}
exports.next = () => {
  return `< [ OTRO ] >`
}
exports.leds = (prefix) => {
  return `*Ejemplo de uso : ${prefix}ledst NeKosmic*`
}
exports.sherlok = () => {
  return `*[ ! ] _Que mensaje desea buscar dentro de este grupo?_*`
}
exports.sherlok2 = () => {
  return `*Encontrado* `
}
exports.sherlok3 = () => {
  return `*mensajes similares* \n\n_Recolectando..._ `
}
exports.sherlok4 = () => {
  return `*[ ! ] _Lo ciento no encontré similitudes para este mensaje_*`
}
exports.letter = () => {
  return `*[ ! ] _Que deseas buscar?_*`
}
exports.playaudi = (prefix) => {
  return `Ejemplo: ${prefix}play mtc s3rl`
}
exports.getmen = () => {
  return `*[ ! ] Responda una imagen que contenga un comentario*`
}
exports.yaesta = () => {
  return `*[ ! ] Esta opción ya estuvo activa en el grupo*`
}
exports.levelon = () => {
  return `Nivel activado en el grupo `
}
exports.leveloff = () => {
  return `Nivel desactivado en el grupo `
}
exports.banichat = () => {
  return `*[ ! ] Este chat fue baneado [ ! ]*`
}
exports.banichat2 = () => {
  return `*Se quitó el ban al chat ✓*`
}
exports.banichat3 = () => {
  return `_*[ BANEAR CHAT ]*_`
}
exports.tutobot = () => {
  return `[ TUTORIAL ✓]`
}
exports.useaudi = () => {
  return `Responda un audio :3`
}
exports.analice = () => {
  return `💉Calculando su porcentaje, aguarde...🔬`
}
exports.toper = () => {
  return `Que top quieres :3?`
}
exports.toper2 = (pushname) => {
  return ` 》 *Gracias por ver* 《 \nGenerador del top ${pushname}`
}
exports.boda = (pushname) => {
  return `${pushname} esta realizando una boda\n\n*Generando pareja...*`
}
exports.rulett = (pushname) => {
  return `*[Girando ruleta]*\n\n_${pushname} que la suerte te acompañe_ ☯️`
}
exports.neonime = () => {
  return `Resultados de últimos lanzamientos anime `
}
exports.wttpad = () => {
  return `Resultados de wattpad :`
}
exports.banderaa = () => {
  return `*nombre de esta bandera?* `
}
exports.comousar = () => {
  return `*[ ! ] Por favor use correctamente el comando*\n\nEjemplo de uso : `
}
exports.sinnet = (prefix) => {
  return `*[ ! ] El bot quedo sin internet (>﹏<)*`
}
exports.moden = () => {
  return `*[ MODOS ] ✓*\n_OFFLINE / ONLINE_ `
}
exports.moden2 = () => {
  return `OFFLINE 🚫`
}
exports.moden3 = () => {
  return `ONLINE 🌐`
}
exports.moden4 = () => {
  return `*[ MODO ENLINEA ACTIVADO]* 🌐\n_Ahora todos los comandos estan disponibles ✓_ `
}
exports.moden5 = () => {
  return `*[ MODO SINLINEA ACTIVADO]* 🚫\n_Ahora solo algunos comandos estaran disponibles ✓_`
}
exports.fotogp = () => {
  return `Se cambio el perfil del grupo ✓`
}
exports.banearr = () => {
  return `*[ ! ] Etiquete al participante que desea banear*`
}
exports.sexy7w7 = () => {
  return `Sexy El Que Lo Lea 7w7r`
}
exports.infobot = (execc, apodox, servery, browserr, conectin, procesor, celular, osversi, modelo, memory, infomcc, infomnc, wasapv, hitshoy, misgp, mispriv, mischat, osarch, myhost) => {
  return `》 *BOT : (activo)* 《    ‏  ‌    ‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‏  ‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌
┏─━─━━─━─━━─━─┓
┣➪ *Version del bot : _5.0.0_*
┣➪ *Hits de hoy : _${hitshoy}_*
┣➪ *Tiempo de ejecucion : _${execc}_*
┣➪ *Chat privados : _${mischat}_*
┣➪ *Grupos chat : _${misgp}_*
┣➪ *Total de chats : _${mispriv}_*
┣➪ *Apodo en Whatsapp : _${apodox}_*
┣➪ *Servidor Web Whatsapp : _${servery}/Baileys_*
┣➪ *Buscador : _${browserr}_*
┣➪ *Velocidad de conexion: _${conectin}_*
┣➪ *Velocidad de procesamiento : _${procesor}_*
*EJECUTANDOSE DESDE*
┣➪ *Dispositivo: _${celular}_*
┣➪ *Version del sistema Operatico OS : _${osversi}_*
┣➪ *Modelo del dispositivo : _${modelo}_*
┣➪ *Plataforma : _${osarch}_*
┣➪ *Host : _${myhost}_*
┣➪ *RAM : Usado _${memory}_*
┣➪ *MCC código de país del móvil : _${infomcc}_*
┣➪ *MNC Código de red móvil : _${infomnc}_*
┣➪ *Version de Whatsapp : _${wasapv}_*
┗─━─━ 「 ✵ 」━━─━─┛\n⇝ *Cariositox-BOT* \n➣ __\nModos de conexion: con paquete de MBS o con promocion de Whatsapp\n▪︎ ~Fecha de inicio: 2020/09/01~`
}
exports.animes = () => {
  return `*[ ! ] Modo anime ta apagado >_<*`
}
exports.animes2 = () => {
  return `Se activo el modo anime en el grupo `
}
exports.animes3 = () => {
  return `Se desactivo el modo anime en el grupo  `
}
exports.animes4 = () => {
  return `_*[MODO ANIME]*_`
}
exports.fullmo = () => {
  return `*[ ! ] El modo full esta desactivado*`
}
exports.fullmo2 = () => {
  return `Se activo modo full en el grupo`
}
exports.fullmo3 = () => {
  return `Se desactivo el modo full en el grupo`
}
exports.fullmo4 = () => {
  return `_*[MODO FULL]*_`
}
exports.nsfw = () => {
  return `*[ ! ] El modo rikolino esta desactivado*`
}
exports.nsfw2 = () => {
  return `Modo H activado en el grupo`
}
exports.nsfw3 = () => {
  return `Modo H desactivado en el grupo`
}
exports.nsfw4 = () => {
  return `_*[MODO H 7w7]*_`
}
exports.wame = (sender, pushname) => {
  return `⚡ *LINKS DE TU NUMERO* ⚡   ‏  ‌   ‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‏  ‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌   ‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‏  ‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌‌\n\n*_Solicitado por_ : 》 ${pushname}《*\n\n▪︎ *Su link directo de whatsapp es* :\n\n▪︎ *https://wa.me/${sender.split("@s.whatsapp.net")[0]}*\n\n*O puede usar este otro*\n\n▪︎ *https://api.whatsapp.com/send?phone=${sender.split("@")[0]}* \nUwU`
}
exports.cgpent = (prefix) => {
  return `*En que tiempo desea cerrar el grupo?*\n\n_Ejemplo de uso: ${prefix}cgpen 10 segundos_`
}
exports.cgpent2 = () => {
  return `*Porfavor seleccione:*\n_segundos_\n_minutos_\n_horas_\n\n*Ejemplo*\n10 segundos`
}
exports.cgpent3 = (sender) => {
  return `*Cerrado Temporizado _Accion ejecutada por @${sender.split("@")[0]}_*\nSolo los administradores podran usar el chat`
}
exports.agpt = (sender) => {
  return `*Grupo abierto por _@${sender.split("@")[0]}_*⚡\nAhora *todos los participantes* pueden enviar sus mensajes`
}
exports.namegp = () => {
  return `*[Se cambio el name del grupo] ✓*`
}
exports.namegp2 = () => {
  return `*Cual sera el nuevo nombre del grupo?*`
}
exports.descgp = () => {
  return `*[Se cambio la descripcion del grupo con etzito] ✓*`
}
exports.descgp2 = () => {
  return `*Cual sera la nueva descripción del grupo?*`
}
exports.niuperfil = (prefix) => {
  return `*_Porfavor responda una imagen ya enviada con el comando_ ${prefix}perfilbot*`
}
exports.niuperfil2 = () => {
  return `*Ahora tengo un nuevo perfil :3*`
}
exports.infogp = (nmGp, dcGp, admGp, itgGp) => {
  return `*[[INFORMACIÓN DEL GRUPO]]*.    ‏  ‌    ‏  ‌.\n\n*Nombre actual del grupo* : ${nmGp}\n\n*Total de administradores* : ${admGp} _AdminGods_\n\n*Total de integrantes* : ${itgGp} _Besto Integrantes UwU_\n\n*Descripcion* : ${dcGp}`
}
exports.coowner = () => {
  return `*y actual dueño del bot*\n\n_Apoyale en sus redes sociales_ : `
}
exports.clonerr = (prefix) => {
  return `*Etiquete a un contacto que quiere copiar su foto de perfil*\n\n*EJEMPLO:*\n ${prefix}fotocloner @NumeroDeUsuario`
}
exports.clonerr2 = () => {
  return `*Etiqueta a un integrante del grupo!*`
}
exports.clonerr3 = () => {
  return `*Foto de perfil actualizada, usando foto de perfil de*`
}
exports.clonerr4 = () => {
  return `*[ ! ] ~BRUH~, No c pudo cambiar foto de perfil, puede ser por que ala persona que etiqueto no tiene foto de perfil ._.XD*`
}
exports.ownergp = () => {
  return `*Creador y propietario del grupo es :* `
}
exports.aaudio = () => {
  return `*[ ! ] Responda un video enviado recientemente* `
}
exports.aaudio2 = () => {
  return `*[X] lo siento no pude convertir el video a audio [X]* `
}
exports.ideh = () => {
  return `*Si no recibe nada dentro de 10 segundos reintente nuevamente :)*`
}
 
